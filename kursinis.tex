...
Buvo pasirinkta atlikti savybių modeliavimą remiantis procesu aprašytu \ref{procesas} skyriuje. Savybių modeliavimas buvo pradėtas nuo srities nusistatymo (plačiau \ref{sritis} Sritis), tada buvo surinkta informacija reikalinga srities modelio sudarymui ir savybių išskyrimui (plačiau \ref{informacija} ...), turint visą reikiamą informaciją buvo galima išskirti savybes (palčiau \ref{isskyrimas} Savybių išskyrimas), sekantis žingsnis buvo modelio sudarymas (plačiau \ref{modelis} Savybių modelis) ir jo validacija (plačiau \ref{validacija} Savybių modelio validacija).

\subsection{Sritis} \label{sritis}

%Pirmiausia buvo išskirta išmaniųjų kontraktų sritis, tada susiaurinta iki pirminio kriptovaliutų platinimo išmaniųjų kontraktai. {Citata, kad išmanieji kontraktai neveikia išorinio pasaulio}. Todėl nereikia analizuoti poveikio išorinėms esybėms ir sąryšio su jomis. 

\subsection{Reikalingų dokumentų surinkimas} \label{informacija}
\subsection{Savybių išskyrimas} \label{isskyrimas}
